Shader "DELTation/Custom/Vertex Animation"
{
    Properties
    {
        [MainTexture]
        ...
        [CustomProperty] _FreqY ("Frequency (Y)", Float) = 10
        [CustomProperty] _FreqX ("Frequency (X)", Float) = 15
        [CustomProperty] _Amplitude ("Amplitude", Float) = 0.05
...
input.positionOS.xyz += input.normalOS * _Amplitude * sin(input.positionOS.y * _FreqY + _Time.y) * cos(input.positionOS.x * _FreqX + _Time.y); \
