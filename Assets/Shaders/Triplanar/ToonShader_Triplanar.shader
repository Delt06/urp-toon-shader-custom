Shader "DELTation/Custom/Triplanar"
{
    Properties
    {
        [MainTexture]
        ...
        [NoScaleOffset] [CustomProperty] _Y ("Y", 2D) = "" {}
        [NoScaleOffset] [CustomProperty] _X ("X", 2D) = "" {}
        [NoScaleOffset] [CustomProperty] _Z ("Z", 2D) = "" {}
...
float4 y_sample = SAMPLE_TEXTURE2D(_Y, sampler_Y, input.positionWSAndFogFactor.xz) * abs(input.normalWS.y); \
float4 x_sample = SAMPLE_TEXTURE2D(_X, sampler_X, input.positionWSAndFogFactor.yz) * abs(input.normalWS.x); \
float4 z_sample = SAMPLE_TEXTURE2D(_Z, sampler_Z, input.positionWSAndFogFactor.xy) * abs(input.normalWS.z); \
float3 tri_w = abs(input.normalWS); \
albedo *= (y_sample * tri_w.y + x_sample * tri_w.x + z_sample * tri_w.z); \
