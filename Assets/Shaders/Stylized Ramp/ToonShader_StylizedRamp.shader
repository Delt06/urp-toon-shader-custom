Shader "DELTation/Custom/Stylized Ramp"
{
    Properties
    {
        [MainTexture]
        ...
        [CustomProperty] _RampNoise ("Ramp Noise", 2D) = "gray" {}
        [CustomProperty] _RampNoiseIntensity ("Ramp Noise Intensity", Range(0,10)) = 0.1
...
SAMPLER(sampler_RampNoise); \
            ...
            // TOON_SHADER_HOOK_RAMP_BRIGHTNESS
             #define TOON_SHADER_HOOK_RAMP_BRIGHTNESS \
half2 noise_uv = GetNormalizedScreenSpaceUV(position_cs); \
noise_uv = apply_tiling_offset(noise_uv, _RampNoise_ST); \
noise_uv.x *= _ScreenParams.x / _ScreenParams.y; \
half noise = SAMPLE_TEXTURE2D(_RampNoise, sampler_RampNoise, noise_uv); \
